...
function EvenementPageInner() {
  ...
  useEffect(() => {
    ...
    const enregistrerVue = async () => {
      const { data: { session } } = await supabase.auth.getSession()
      fetch(`/api/evenements/${ev.id}/vue`, {
        method: 'POST',
        headers: {
  ...
  const handleLike = async () => {
    ...
    if (session?.user?.id) {
      ...
      if (liked) {
        await supabase.from('favoris').delete().eq('evenement_id', id).eq('user_id', userId)
  ...
  const handleSeraiLa = async () => {
    ...
    if (seraiLa) {
      ...
      await supabase.from('participations').insert([{
        evenement_id: id, session_id: sessionId, user_id: userId || null,
      }])
  ...
  const handleSignalement = async () => {
    ...
    const { error } = await supabase.from('signalements').insert([{
      evenement_id: ev.id, raison: raisonSignalement, user_id: session?.user?.id ?? null,
    }])
    if (error) { setErreurSignalement('Une erreur est survenue. Réessaie plus tard.'); return }
  ...
  const handleNouvelleReponse = (parentId: string, reponse: Commentaire) => {
    setCommentaires(prev => prev.map(c => {
      if (c.id === parentId) return { ...c, reponses: [...(c.reponses || []), reponse] }
      return c
    }))
  }
  ...
  const trackerPartage = async (canal: string) => {
    const { data: { session } } = await supabase.auth.getSession()
    fetch(`/api/evenements/${ev.id}/partage`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(session?.access_token ? { Authorization: `Bearer ${session.access_token}` } : {}),
  ...
  return (
    <main style={{ minHeight: '100dvh', background: '#F7F2E8', color: '#1A1410' }}>
      ...
      {carteVisuelleouverte && (
        <CarteVisuelle
          evenement={{ id: ev.id, titre: ev.titre, lieu: ev.lieu, date: ev.date, date_fin: ev.date_fin ?? undefined, image_url: ev.image_url ?? undefined }}
              ...
              <>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  {propositionForm.champ_modifie === 'description' ? (
                    ...
                    <input type={propositionForm.champ_modifie === 'date' ? 'date' : 'text'}
      ...
      <div style={{ maxWidth: isDesktop ? 1200 : '100%', margin: '0 auto', display: isDesktop ? 'grid' : 'block', gridTemplateColumns: isDesktop ? '480px 1fr' : undefined, alignItems: 'start', minHeight: isDesktop ? '100dvh' : undefined }}>

        {/* Colonne gauche — Image sticky */}
        <div style={{ position: isDesktop ? 'sticky' : 'static', top: 0, height: isDesktop ? '100dvh' : 280, overflow: 'hidden' }}>
          <img src={ev.image_url || imageAuto || getEventImage(null, ev.categorie)} alt={ev.titre}
            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          {isDesktop && (
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, transparent 60%, #F7F2E8 100%)' }} />
          )}
        ...
        <div style={{ padding: isDesktop ? '40px 48px 80px' : '24px 16px 64px', maxWidth: isDesktop ? '100%' : 680, margin: isDesktop ? 0 : '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            ...
            {isAdmin && (
              <button onClick={async () => {
                if (ev.parent_id) {
                  const choix = window.confirm('Mettre hors ligne toutes les occurrences futures ?\n\nOK = Toutes les occurrences\nAnnuler = Seulement cet événement')
                  if (choix) {
                    await supabase.from('evenements').update({ statut: 'hors_ligne' }).eq('parent_id', ev.parent_id)
                    await supabase.from('evenements').update({ statut: 'hors_ligne' }).eq('id', ev.parent_id)
                  } else {
          ...
          <div style={{ marginBottom: 16 }}>
            <button onClick={handleSeraiLa} disabled={loadingParticipation} style={{
              ...
              <span>{seraiLa ? `${expressionChoisie} ✓` : t.evenement.jeSeraiLa}</span>
              {nbParticipants > 0 && (
            ...
            {seraiLa && (
              <button onClick={() => setCarteVisuelleouverte(true)} style={{
                ...
                {t.evenement.creerCarteVisuelle}
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 32 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <button onClick={handleLike} style={{
                ...
                <span>{t.evenement.jaime}{nbLikes > 0 ? ` ${nbLikes}` : ''}</span>
              </button>
              <button onClick={() => setSignalementModal(true)} style={{
                display: 'flex', alignItems: 'center', gap: 5,
                background: 'white', border: '1px solid #E8E0D0',
                borderRadius: 999, padding: '8px 12px', cursor: 'pointer', color: '#555', fontSize: 12, whiteSpace: 'nowrap',
              }}>
                <span style={{ fontSize: 14 }}>⚠️</span><span>{t.evenement.signalerCourt}</span>
              </button>
              {userProfile && (
                <button onClick={() => {
                  ...
                  <span style={{ fontSize: 14 }}>✏️</span><span>{t.evenement.corrigerCourt}</span>
                </button>
              )}
              {ev.user_id !== userProfile?.id && (
                <button onClick={() => { if (!userProfile) { window.location.href = '/login?redirect=/evenement/' + ev.id; return } setClaimModal(true) }}
                  style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'white', border: '1px solid #E8E0D0', borderRadius: 999, padding: '8px 12px', cursor: 'pointer', color: '#555', fontSize: 12, whiteSpace: 'nowrap' }}>
                  <span style={{ fontSize: 14 }}>🔑</span><span>{t.evenement.cestMonEvenement}</span>
                </button>
              )}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <a href={urlWhatsapp} target="_blank" onClick={() => trackerPartage('whatsapp')} title="WhatsApp" style={{ width: 38, height: 38, borderRadius: '50%', background: '#25D366', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flexShrink: 0 }}>
              ...
              <a href={urlFacebook} target="_blank" onClick={() => trackerPartage('facebook')} title="Facebook" style={{ width: 38, height: 38, borderRadius: '50%', background: '#1877F2', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flexShrink: 0 }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="white"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href={'https://twitter.com/intent/tweet?text=' + encodeURIComponent(t.evenement.decouvreEvenementSur + ' ' + ev.titre) + '&url=' + encodeURIComponent(urlEvenement)} target="_blank" onClick={() => trackerPartage('x')} title="X" style={{ width: 38, height: 38, borderRadius: '50%', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flexShrink: 0 }}>
          ...
          <div style={{ marginBottom: 48 }}>
            <h2 style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 20, color: '#1A1410' }}>{t.evenement.commentaires}</h2>
            <CommentaireForm
              evenementId={ev.id}
              userProfile={userProfile}
          ...
          {occurrences.length > 0 && (
            <div style={{ marginBottom: 32 }}>
              <h2 style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 16, color: '#1A1410' }}>🔄 Prochaines occurrences</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {occurrences.map(occ => (
                  <a key={occ.id} href={'/evenement/' + occ.id} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    background: 'white', border: '1px solid #E8E0D0',
                    borderRadius: 10, padding: '12px 16px', textDecoration: 'none', color: '#1A1410',
                  }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        ...
                        {occ.heure_debut && <p style={{ color: '#8C5A40', fontSize: 12 }}>🕐 {occ.heure_debut}</p>}
                      </div>
                    </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {similaires.map(sim => (
                  <a href={'/evenement/' + sim.id} key={sim.id} style={{ display: 'flex', gap: 12, background: 'white', border: '1px solid #E8E0D0', borderRadius: 12, padding: 12, textDecoration: 'none', color: '#1A1410' }}>
                    <img src={getEventImage(sim.image_url, sim.categorie)} alt={sim.titre}
                      style={{ width: 64, height: 64, objectFit: 'cover', borderRadius: 8, flexShrink: 0 }}
                      onError={(e) => { if (sim.image_url) { (e.target as HTMLImageElement).style.display = 'none'; return; } const img = e.target as HTMLImageElement; const fb = getEventImage(null, sim.categorie); if (img.src !== fb) img.src = fb; else img.style.display = 'none' }} />

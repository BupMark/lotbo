...
// ── Système de badges (synchronisé avec app/profil/page.tsx) ─────────────────
...
/** Retourne le badge le plus élevé atteint pour un nombre donné */
...
/** Retourne le badge nouvellement débloqué si on est passé de avant à apres */
function detecterNouveauBadge(avant: number, apres: number, badges: Badge[]): Badge | null {
  ...
  if (!badgeAvant) return badgeApres // premier badge
  if (badgeApres.id !== badgeAvant.id) return badgeApres // nouveau palier
  return null
}
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          boxShadow: '0 0 60px rgba(212,168,32,0.15)',
        }}>
          ...
          {/* Titre */}
          <p style={{ color: '#D4A820', fontSize: 12, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 8 }}>
            🏅 Badge débloqué !
          </p>
          <h2 style={{ color: '#F7F2E8', fontSize: 26, fontWeight: 'bold', fontFamily: 'serif', fontStyle: 'italic', marginBottom: 8 }}>
            {badge.label}
          </h2>
          <p style={{ color: '#8C5A40', fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>
            {isContrib
              ? `Félicitations ! Tu viens d'atteindre ta ${ordinal} contribution sur LOTBO !`
              : `Félicitations ! Tu viens de soumettre ton ${ordinal} événement sur LOTBO !`
            }
          </p>
...
// ── Carte interactive avec drag pin ──────────────────────────────────────────
...
export default function AjouterEvenement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ── Compter AVANT insertion pour détecter nouveau badge ──
    const userId = session?.user?.id || ''
    const { count: nbAvant } = await supabase
      .from('evenements')
      .select('id', { count: 'exact', head: true })
      .eq('user_id', userId)
      .eq('soumis_en_tant_que', choix)
    ...
    const { count: nbApres } = await supabase
      .from('evenements')
      .select('id', { count: 'exact', head: true })
      .eq('user_id', userId)
      .eq('soumis_en_tant_que', choix)
    ...
    setSuccesData({
      ...
      nbContributions: apres,
      evenementId:     inserted?.id,
      nouveauBadge:    nouveauBadge || undefined,
    })

    setSucces(true)
    // Afficher la popup badge en premier si applicable
    if (nouveauBadge) setShowBadgePopup(true)
  }

  // ── Fermer la popup badge et aller sur le profil ──────────────────────────
  const handleContinuerApresBadge = () => {
    setShowBadgePopup(false)
    // Redirige vers les badges du profil
  ...
  if (succes) {
    ...
    return (
      <main style={{ minHeight: '100dvh', background: '#1A1410', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px' }}>
        ...
        {/* Popup badge — s'affiche par-dessus */}
        ...
        <div style={{ maxWidth: 480, width: '100%' }}>
          ...
          <h2 style={{ color: '#F7F2E8', fontSize: 24, fontWeight: 'bold', textAlign: 'center', marginBottom: 12 }}>
            {isContrib
              ? `Félicitations pour ta ${ordinal} contribution !`
          ...
          {/* Badge débloqué — rappel discret si popup déjà vue */}

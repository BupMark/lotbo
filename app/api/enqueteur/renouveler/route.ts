...
export async function GET(request: Request) {
  ...
  if (!id) {
    return new NextResponse(pageConfirmation('Lien invalide', 'Ce lien de renouvellement est invalide ou incomplet.'), { status: 400, headers: { 'Content-Type': 'text/html' } })
  }
  ...
  const now = new Date()
  const dansUnAn = new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000)

  const { data, error } = await admin
    .from('enqueteurs')
    .update({
      consent_revue_at: now.toISOString(),
      consent_revue_due: dansUnAn.toISOString(),
      renouvellement_notifie_le: null,
      fiche_masquee: false,
      masque_le: null,
    })
    .eq('id', id)
  ...
  return new NextResponse(pageConfirmation('Participation confirmée ✅', 'Ta fiche enquêteur reste active pour une année supplémentaire. Merci de continuer à contribuer !'), { status: 200, headers: { 'Content-Type': 'text/html' } })
}

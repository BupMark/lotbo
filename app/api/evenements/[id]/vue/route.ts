...
export async function POST(
  ...
  try {
    ...
    const admin = makeAdminClient()
    const { error } = await admin.from('vues_evenements').insert([{
      evenement_id: evenementId,
      user_id: userId,
      session_id: userId ? null : sessionId,
    }])

    // Code 23505 = violation de contrainte unique — déjà vu, pas
    // une vraie erreur, comportement attendu du dédoublonnage

  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: org, error } = await supabase
        .from('organisations')
        .select('id, nom, slogan, description, ville, pays, site_web, email_contact, telephone, visible, owner_id')
        .eq('slug', slug)
        .single()

      if (error || !org) { router.push('/organisations'); return }
  ...
  return (
    <main style={{ minHeight: '100dvh', background: '#F7F2E8', padding: '24px 16px 80px' }}>
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 'bold', color: '#1A1410', marginBottom: 28 }}>
          ⚙️ Paramètres — {form.nom}
        </h1>

        {/* Section Informations */}
        <div style={{ background: 'white', borderRadius: 16, padding: 20, border: '1px solid #E8E0D0', marginBottom: 20 }}>
        ...
        {/* Section Visibilité */}
        <div style={{ background: 'white', borderRadius: 16, padding: 20, border: '1px solid #E8E0D0', marginBottom: 20 }}>
          <h2 style={{ fontSize: 14, fontWeight: 'bold', color: '#1A1410', marginBottom: 16 }}>👁️ Visibilité</h2>
          ...
          <p style={{ fontSize: 12, color: '#8C5A40', marginTop: 8 }}>
            Si activé, votre organisation apparaît dans la page /organisations et est découvrable par tous.
          </p>
        </div>

        {/* Bouton sauvegarder */}
        <button onClick={handleSave} disabled={saving}
          style={{ width: '100%', background: saving ? 'rgba(200,67,26,0.6)' : '#C8431A', color: 'white', border: 'none', borderRadius: 12, padding: '14px', fontSize: 15, fontWeight: 'bold', cursor: saving ? 'not-allowed' : 'pointer', marginBottom: 12 }}>
        ...
        {/* Section Danger */}
        <div style={{ background: 'white', borderRadius: 16, padding: 20, border: '1px solid rgba(200,67,26,0.3)', marginTop: 12 }}>
